...
export default function Home() {
  ...
  return (
    <>
      <Navbar />
      <main className={`w-100 h-screen flex flex-col md:flex-row w-full pt-[50px] md:pt-[75px] relative ${theme === 'dark' ? 'dark-mode' : 'light-mode'}`}>
        {/* Pass links and links2 to SideBar */}
        <SideBar links={links} links2={links2} />
        <section className={`mt-1 w-full md:w-[83%] bg-[#F4F4F4] md:ml-[16.6%] ${theme === 'dark' ? 'dark-mode-section' : 'light-mode-section'}`}>
          <div className={`flex flex-wrap justify-between gap-[25px] md:gap-[0px] md:gap-y-5 py-[30px] px-[30px] ${theme === 'dark' ? 'dark-mode-content' : 'light-mode-content'}`}>
            {loading ? (
              // Render card skeleton if loading
              Array.from({ length: 10 }).map((_, index) => (
                <CardSkeleton key={index} />
