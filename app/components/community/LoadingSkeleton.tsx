...
const LoadingSkeleton: React.FC<{ type: "discussion" | "contributor" }> = ({ type }) => {
  ...
  const skeletonClasses = `animate-pulse bg-${theme === "dark" ? "gray-700" : "white"} border rounded-md p-4`;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {Array.from({ length: 3 }).map((_, index) => (
          <div key={index} className={skeletonClasses}>
            <div className={`h-4 bg-${theme === "dark" ? "gray-600" : "gray-300"} rounded mb-2 w-3/4`}></div>
            <div className={`h-3 bg-${theme === "dark" ? "gray-600" : "gray-300"} rounded mb-2 w-full`}></div>
            <div className={`h-3 bg-${theme === "dark" ? "gray-600" : "gray-300"} rounded mb-2 w-5/6`}></div>
